...
import { useAnalyzeCompetitorMutation, SEOCompetitor } from "@/state/api";
export default function CompetitorAnalysis({ competitors: initial }: { competitors: SEOCompetitor[] }) {
  const [competitors, setCompetitors] = useState<SEOCompetitor[]>(initial);
  const [domain, setDomain] = useState("");
  const [analyzeCompetitor, { isLoading }] = useAnalyzeCompetitorMutation();
  const handleAdd = async () => {
    if (!domain.trim()) return;
    try {
      const result = await analyzeCompetitor({ domain: domain.trim() }).unwrap();
  ...
  return (
    <div>
      <div className="flex gap-2 mb-5">
        ...
        <button onClick={handleAdd} disabled={isLoading} className="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors">
          {isLoading ? "..." : "+ Add"}
        </button>
      </div>
              ...
              <tr key={c.domain} className="border-b border-gray-100 dark:border-gray-700 last:border-0">
                ...
                <td className="py-3 px-3">
                  <div className="flex flex-wrap gap-1">
                    {c.strengths.map(s => <span key={s} className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full">{s}</span>)}
                  </div>
                </td>
              </tr>
            ))}
