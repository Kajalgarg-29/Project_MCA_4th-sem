...
import { ChevronLeft, ChevronRight, Calendar, Clock, AlertCircle, X } from "lucide-react";
...
function getBarStyle(startDate: Date | null, endDate: Date | null, viewStart: Date, totalDays: number) {
  ...
  const effectiveEnd   = endDate || new Date(effectiveStart.getTime() + 7 * 86400000);
  const viewEnd        = new Date(viewStart.getTime() + totalDays * 86400000);
  ...
  const startOffset   = Math.max(0, (clampedStart.getTime() - viewStart.getTime()) / 86400000);
  const duration      = Math.max(1, (clampedEnd.getTime() - clampedStart.getTime()) / 86400000);
  ...
  return (
    <DashboardWrapper>
      <div className="p-4 sm:p-6 max-w-full">
        ...
        <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-3 mb-5 sm:mb-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden">
              <button onClick={prevMonth} className="p-2 hover:bg-gray-50 transition"><ChevronLeft size={15} /></button>
              <span className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm font-medium text-gray-700 min-w-[110px] sm:min-w-[130px] text-center">
                {FULL_MONTHS[viewMonth]} {viewYear}
              </span>
              <button onClick={nextMonth} className="p-2 hover:bg-gray-50 transition"><ChevronRight size={15} /></button>
            </div>
          </div>
        </div>

        {/* ── Summary Cards — 2×2 mobile, 4-col md+ ── */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-5 sm:mb-6">
          {[
            ...
            <div key={s.label} className="bg-white rounded-xl p-3 sm:p-4 shadow-sm border border-gray-100">
              <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center ${s.color} mb-2`}>
                <s.icon size={16} />
              </div>
        ...
        {/* ── Gantt Chart ── */}
        {projects.length === 0 ? (
          <div className="bg-white rounded-xl p-10 sm:p-16 text-center shadow-sm border border-gray-100">
            <Calendar size={40} className="mx-auto mb-3 text-gray-200" />
            <p className="text-gray-400 font-medium text-sm">No projects yet</p>
            <p className="text-gray-300 text-xs mt-1">Create projects with start and end dates to see the timeline</p>
          </div>
        ) : (
            ...
            <div className="overflow-x-auto">
              <div style={{ minWidth: "640px" }}>
                ...
                <div className="flex border-b border-gray-100">
                  <div className="w-48 sm:w-64 shrink-0 px-3 sm:px-4 py-3 border-r border-gray-100 bg-gray-50">
                  ...
                  <div className="w-24 sm:w-28 shrink-0 px-3 py-3 bg-gray-50 border-l border-gray-100">
                ...
                {projects.map((project: any, idx: number) => {
                  ...
                  return (
                    ...
                    >
                      {/* Project name column */}
                      <div className="w-48 sm:w-64 shrink-0 px-3 sm:px-4 py-3 border-r border-gray-100 flex items-center gap-2">
                        <div className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full ${color.bg} shrink-0`} />
                        <div className="min-w-0">
                          <p className="text-xs sm:text-sm font-medium text-gray-800 truncate">{project.name}</p>
                      ...
                      <div className="flex-1 relative py-3 px-0">
                        {/* Weekend tint + day grid lines */}
                        <div className="absolute inset-0 flex pointer-events-none">
                          {dayHeaders.map(day => {
                            const date      = new Date(viewYear, viewMonth, day);
                            const isWeekend = date.getDay() === 0 || date.getDay() === 6;
                            return <div key={day} className={`flex-1 border-r border-gray-50 last:border-0 ${isWeekend ? "bg-gray-50/50" : ""}`} />;
                          })}
                        </div>

                        {/* Today line */}
                        {todayOffset !== null && (
                          <div className="absolute top-0 bottom-0 w-px bg-blue-400 z-10 pointer-events-none" style={{ left: `${todayOffset}%` }} />
                        )}

                        {/* Project bar */}
                        {barStyle ? (
                          <div className="absolute top-1/2 -translate-y-1/2 h-6 sm:h-7 rounded-full overflow-hidden shadow-sm" style={barStyle}>
                            <div className={`h-full w-full ${color.light} border ${color.border} relative flex items-center`}>
                              <div className={`absolute left-0 top-0 bottom-0 ${color.bg} opacity-40 rounded-full transition-all`} style={{ width: `${progress}%` }} />
                              <span className={`relative z-10 px-2 text-xs font-semibold ${color.text} truncate`}>{project.name}</span>
                            </div>
                          </div>
                        ) : (
                          <div className="flex items-center justify-center h-full">
                            <span className="text-xs text-gray-300 italic">{!project.startDate && !project.endDate ? "No dates" : "Outside view"}</span>
                          </div>
                        )}
                      </div>

                      {/* Status column */}
                      <div className="w-24 sm:w-28 shrink-0 px-2 sm:px-3 py-3 border-l border-gray-100 flex items-center">
                        <span className={`text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full font-medium whitespace-nowrap ${status.color}`}>{status.label}</span>
                      </div>
                    </div>
                  );
                })}
        ...
        {selectedProject && (
          <div className="mt-4 bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5">
            <div className="flex justify-between items-start mb-4 gap-3">
              <div className="min-w-0">
                <h3 className="font-semibold text-gray-800 text-base sm:text-lg truncate">{selectedProject.name}</h3>
                {selectedProject.description && (
                  <p className="text-xs sm:text-sm text-gray-400 mt-0.5">{selectedProject.description}</p>
                )}
              </div>
              <button onClick={() => setSelectedProject(null)} className="p-1 text-gray-300 hover:text-gray-500 transition shrink-0">
                <X size={18} />
              </button>
            </div>

            {/* Detail cards — 1-col on mobile, 3-col on sm+ */}
              ...
              <div className="bg-gray-50 rounded-xl p-3">
                ...
                <p className="text-sm font-medium text-gray-700">
                  {selectedProject.endDate
                    ? new Date(selectedProject.endDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })
                    : "Not set"}
                </p>
              </div>
              <div className="bg-gray-50 rounded-xl p-3">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <div className="flex-1 bg-gray-200 rounded-full h-2">
                    <div className="bg-blue-500 h-2 rounded-full transition-all" style={{ width: `${getProgress(selectedProject)}%` }} />
                  </div>
                  <span className="text-sm font-medium text-gray-700 shrink-0">{getProgress(selectedProject)}%</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {projectsWithoutDates.map((p: any) => (
                <span key={p.id} className="text-xs bg-gray-100 text-gray-500 px-3 py-1.5 rounded-full">
                  {p.name}
                </span>
              ))}
            </div>
