  ...
  if (list.length === 0) {
    return (
      <div className="text-center py-10 text-gray-400 text-sm">
        No content gap data available. Run an analysis first.
      </div>
    );
  }
      ...
      {list.map((gap, i) => (
        <div key={i} className="p-3 bg-white border border-gray-100 rounded-xl shadow-sm">
          <div className="flex justify-between items-start">
            ...
            {gap.opportunity && (
              <span className="text-xs bg-purple-50 text-purple-600 px-2 py-0.5 rounded-full ml-2 shrink-0">
                {gap.opportunity}
              </span>
            )}
          </div>
          {gap.difficulty !== undefined && (
            <p className="text-xs text-gray-400 mt-1">Difficulty: {gap.difficulty}/100</p>
          )}
        </div>
      ))}
    </div>
