...
export default function KeywordRecommendations({ keywords }: { keywords: any[] }) {
  const avgVol = keywords.length ? Math.round(keywords.reduce((a:number, k:any) => a + k.volume, 0) / keywords.length) : 0;
  const highPri = keywords.filter((k:any) => k.priority === "high").length;
  return (
    <div>
